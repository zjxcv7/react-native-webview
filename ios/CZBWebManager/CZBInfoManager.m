...
+ (BOOL)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction {
    ...
    if ([navigationAction.request.URL.absoluteString hasPrefix: @"alipay"]) {
        NSMutableString *urlString = [[NSMutableString alloc] initWithString:navigationAction.request.URL.absoluteString];
        if ([urlString containsString:@"fromAppUrlScheme"] && [urlString containsString:@"alipays"]) {
            NSRange range = [urlString rangeOfString:@"alipays"];
            [urlString replaceCharactersInRange:range withString:@"smant"];
        }
...
//            [request setValue:@"msdev.czb365.com://" forHTTPHeaderField:key];
//        }];
